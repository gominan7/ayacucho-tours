...
El sistema DEBE autenticar a los usuarios utilizando su correo electrónico y contraseña registrados a través de Supabase Auth (BR-028). Solo los usuarios con estado activo en el sistema podrán iniciar sesión (BR-003).
...
El sistema DEBE permitir a los usuarios autenticados cerrar su sesión activa para evitar accesos no autorizados.
...
El sistema DEBE proteger todas las rutas privadas (incluyendo `/dashboard` y subrutas) para restringir el acceso únicamente a usuarios autenticados (BR-026, BR-027, BR-028).
